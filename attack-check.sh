...
# Root-check: без root скрипт молча возвращает пустой отчёт (apt-get install,
# iptables, tcpdump требуют root). Лучше fail-fast.
if [ "${EUID:-$(id -u)}" -ne 0 ]; then
  crit "Запусти как root: sudo bash $0"
  exit 1
fi
...
echo
# `${#var}` — это длина строки, а не значение. Тут нужен реальный процент.
if [ "${PKT_TOTAL:-0}" -gt 0 ] && [ "${SYN_COUNT:-0}" -gt $((PKT_TOTAL/2)) ] 2>/dev/null; then
  SYN_PCT=$(( SYN_COUNT * 100 / PKT_TOTAL ))
  crit "SYN-flood: ${SYN_COUNT} SYN из ${PKT_TOTAL} пакетов (${SYN_PCT}%)"
fi
...
hdr "8. ТОП-20 ИСТОЧНИКОВ ПАКЕТОВ (tcpdump, IPv4)"
# `grep " IP "` (с пробелами) ловит ТОЛЬКО v4 type-field tcpdump'а; " IP6 "
# отбрасывается. Раньше `grep "IP"` совпадал с обоими, и v6-адреса попадали
# в pipeline awk -F. где NF<4 их тихо отфильтровывало — статистика v4-only,
# но без явного маркера.
if [ "${PKT_TOTAL}" -gt 0 ]; then
  grep " IP " "${DUMP}" | awk '{print $3}' | \
    awk -F. 'NF>=4 {print $1"."$2"."$3"."$4}' | \
    sort | uniq -c | sort -rn | head -20

  # IPv6 источники — отдельный блок (в v6 нет точек в адресе, awk -F. бесполезен).
  V6_TOTAL=$(grep -c " IP6 " "${DUMP}" 2>/dev/null)
  if [ "${V6_TOTAL:-0}" -gt 0 ]; then
    sub "+IPv6 источники (${V6_TOTAL} пакетов)"
    grep " IP6 " "${DUMP}" | awk '{print $3}' | \
      sed 's/\.[0-9]*$//' | sort | uniq -c | sort -rn | head -10
  fi
fi

# ═════════════════════════ 9. ГЕОГРАФИЯ ═════════════════════════
hdr "9. ГЕОГРАФИЯ (ip-api.com, IPv4)"
